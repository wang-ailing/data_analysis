...
def outstanding_1_detection(data: Series, threshold: float = 0.3, beita: float = 0.7) -> Tuple[Dict[str, Union[int, float]], str]:
    """
    ...
    if count == 1 and outstanding_1_check(data=data, threshold=threshold, beita=beita) == True:
        id_max = data.idxmax()
        value_max = data.max()
        ratio_to_second = value_max // data[data != value_max ].max()
        ratio_to_min = value_max // data.min()
